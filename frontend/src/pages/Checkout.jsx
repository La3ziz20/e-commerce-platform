  ...
  if (cartItems.length === 0) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        ...
        <button className="btn btn-primary btn-hover-anim" style={{ marginTop: '16px' }} onClick={() => navigate('/')}>{t('checkout.return')}</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-xl)', alignItems: 'start' }}>
      <div className="glass-panel" style={{ padding: 'var(--space-xl)' }}>
        <h2 style={{ marginBottom: 'var(--space-lg)' }}>{t('checkout.details')}</h2>
        <form onSubmit={handlePay} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
          <div className="input-group">
            <CreditCard className="input-icon" size={20} />
            <input type="text" placeholder={t('checkout.cardNumber')} className="glass-input" required />
          </div>
          ...
          <button type="submit" className="btn btn-primary btn-hover-anim" style={{ width: '100%', marginTop: 'var(--space-md)' }}>
            {t('checkout.pay')} {cartTotal.toLocaleString('en-TN', { style: 'currency', currency: 'TND' })}
          </button>
        </form>
      </div>
      
      <div className="glass-panel" style={{ padding: 'var(--space-xl)' }}>
        <h3 style={{ marginBottom: 'var(--space-md)' }}>{t('checkout.summary')}</h3>
